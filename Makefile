...
deploy:
	ssh root@89.169.37.68 "cd /var/www/formaflow/backend && git fetch origin && git reset --hard origin/master && composer install --no-dev --optimize-autoloader && php artisan migrate --force"
